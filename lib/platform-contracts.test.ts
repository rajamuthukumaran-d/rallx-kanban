...
describe('delivery contracts', () => {
  it('keeps the service worker on the public shell allowlist', () => {
    ...
    expect(worker).toContain("request.mode === 'navigate' && new URL(request.url).pathname !== '/'");
  ...
  it('ships local install icons and manifest sizes', () => {
    ...
    const manifest = JSON.parse(read('public/manifest.webmanifest')) as { icons: Array<{ sizes: string }> };
  ...
  it('documents explicit migration deployment and ignores build secrets', () => {
    ...
    expect(read('docker-compose.yml')).toContain('DATABASE_URL:?set DATABASE_URL');
  });
});
