...
`app/` is the App Router shell, `components/kanban/` contains the interactive board components, and `test/mock-data.ts` is the replaceable local repository boundary used by the prototype. The Prisma schema is split into `prisma/schema.prisma` for the generator/datasource and one model per file under `prisma/models/`. Child records carry `tenantId`; composite foreign keys bind projects, tickets, memberships, actors, and integrations to the same tenant. PostgreSQL constraints therefore reject cross-tenant references, but application authorization and a backend service are still required.
